function abClassifier = buildAdaBoost(trnX, thX, trnY, iter, tstX, tstY)
if nargin < 5
    tstX = [];
    tstY = [];
end
...
for i = 1:iter
    weakClassifier = buildStump(trnX, thX,trnY, sampleWeight);
    abClassifier.WeakClas{i} = weakClassifier;
    abClassifier.sumWeight(i) = sum(sampleWeight);
    abClassifier.nWC = i;
